...
type StepStatus = "pending" | "in_progress" | "success" | "failed";
...
const WORKFLOW_STEPS: Omit<WorkflowStep, "status">[] = [
  ...
  { id: "summary", label: "Incident summary generated", service: "Luvira Guardian" },
  { id: "slack-post", label: "Slack message posted", service: "Slack" },
  { id: "calendar-create", label: "Calendar event created", service: "Google Calendar" },
];
...
function StepIcon({ status }: { status: StepStatus }) {
  ...
  if (status === "in_progress") {
    return <span className="w-5 h-5 rounded-full border-2 border-[#3bcaca] border-t-transparent animate-spin" />;
  }
  if (status === "failed") {
    return (
  ...
  return <span className="w-5 h-5 rounded-full border border-gray-300 bg-white" />;
}

function PermissionContractModal({ onApprove, onDeny }: { onApprove: () => void; onDeny: () => void }) {
  return (
        ...
        <div className="p-6">
          <div className="flex items-center gap-2.5 mb-5">
            <div className="w-8 h-8 rounded-lg bg-[#3bcaca]/10 border border-[#3bcaca]/20 flex items-center justify-center">
              <svg className="w-4 h-4 text-[#3bcaca]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              <h2 className="text-[15px] font-semibold text-gray-900">Permission Contract</h2>
              <p className="text-[11px] text-gray-400">Review before authorizing</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <section>
              ...
              <ul className="space-y-1">
                {["Read GitLab Incident Issue", "Post Slack Notification", "Schedule Calendar Meeting"].map((action) => (
                  <li key={action} className="flex items-center gap-2 text-[13px] text-gray-600">
            ...
            <section>
              ...
              <p className="text-[13px] text-[#3bcaca] font-medium">Auth0 Token Vault</p>
            </section>
            ...
            <section>
              ...
              <div className="space-y-1.5">
                {[
                  ...
                  { service: "Google Calendar", scope: "…/auth/calendar.events" },
                ].map(({ service, scope }) => (
                  <div key={service} className="flex items-center gap-2">
                    <span className="text-[12px] text-gray-400 w-28 shrink-0">{service}:</span>
                    <span className="text-[12px] font-mono text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded">{scope}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <ul className="space-y-1">
                {["No credentials stored locally", "Tokens are short-lived", "All actions logged"].map((g) => (
                  <li key={g} className="flex items-center gap-2 text-[12px] text-gray-600">
...
function HighRiskModal({ onConfirm, onCancel }: { onConfirm: () => void; onCancel: () => void }) {
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div className="bg-white border border-yellow-200 rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
        ...
        <div className="p-6 text-center">
          ...
          <h2 className="text-[16px] font-semibold text-gray-900 mb-1">High-Risk Action Detected</h2>
          <p className="text-[13px] text-gray-500 mb-1">Additional verification required</p>
          <p className="text-[12px] text-yellow-600 mb-6">Awaiting confirmation on your device&hellip;</p>
...
function ReconnectBanner({ service, onReconnect, onDismiss }: { service: string; onReconnect: () => void; onDismiss: () => void }) {
  return (
    <div className="border border-red-200 bg-red-50 rounded-xl p-4 flex items-start gap-3">
      ...
      <div className="flex-1">
        <p className="text-[13px] font-semibold text-red-700">{service} connection lost</p>
        <p className="text-[12px] text-gray-500 mt-0.5">Reconnect required to continue this workflow.</p>
      </div>
      <div className="flex gap-2 shrink-0">
        <button onClick={onReconnect} className="px-3 py-1.5 text-[12px] font-medium text-white bg-red-500 rounded-lg hover:bg-red-600 transition-colors">
          Reconnect
        </button>
        <button onClick={onDismiss} className="px-3 py-1.5 text-[12px] font-medium text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
          Dismiss
        </button>
      </div>
    </div>
...
export default function AgentConsolePage() {
  ...
  function handleDeny() { setState({ type: "idle" }); }
  ...
  function runStep(steps: WorkflowStep[], index: number) {
    if (index >= steps.length) { setState({ type: "completed", steps }); return; }
    const updated = steps.map((s, i) => i === index ? { ...s, status: "in_progress" as StepStatus } : s);
    setState({ type: "executing", steps: updated, connectionLost: null });
    if (index === 2) {
      executionRef.current = setTimeout(() => {
        setState({ type: "high_risk", steps: updated, pendingStepIndex: index });
      }, 800);
      return;
    }
    const delay = index === 1 ? 1500 : 2000;
    executionRef.current = setTimeout(() => {
      const done = updated.map((s, i) => i === index ? { ...s, status: "success" as StepStatus } : s);
  ...
  function handleHighRiskConfirm() {
    ...
    setState({ type: "executing", steps, connectionLost: null });
    executionRef.current = setTimeout(() => {
      const done = steps.map((s, i) => i === pendingStepIndex ? { ...s, status: "success" as StepStatus } : s);
  ...
  function handleHighRiskCancel() {
    ...
    const failed = state.steps.map((s, i) =>
      i === state.pendingStepIndex ? { ...s, status: "failed" as StepStatus } : s
    );
  ...
  function handleReset() {
    if (executionRef.current) clearTimeout(executionRef.current);
    setState({ type: "idle" });
    setPrompt("Review the incident issue, notify the team, and schedule a follow-up meeting.");
  }
  ...
  const executingSteps =
    state.type === "executing" ? state.steps
    : state.type === "high_risk" ? state.steps
    : state.type === "completed" ? state.steps
    : state.type === "connection_lost" ? state.steps
    : null;

  return (
    <>
      {state.type === "contract" && <PermissionContractModal onApprove={handleApprove} onDeny={handleDeny} />}
      {state.type === "high_risk" && <HighRiskModal onConfirm={handleHighRiskConfirm} onCancel={handleHighRiskCancel} />}

      <div className="px-8 py-8 w-full">
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Agent Console</h1>
        ...
        {state.type === "planning" && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-5 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-[13px] font-semibold text-gray-900">Agent Workflow Plan</p>
            </div>
        ...
        {state.type === "connection_lost" && (
          <div className="mb-5">
            <ReconnectBanner service={state.service} onReconnect={handleReconnect} onDismiss={handleReset} />
          </div>
        )}

        {/* Execution Timeline */}
        {executingSteps && state.type !== "planning" && state.type !== "contract" && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                ...
                <div key={step.id} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    ...
                    {i < executingSteps.length - 1 && (
                      <div className={`w-px flex-1 mt-1 mb-1 min-h-5 ${step.status === "success" ? "bg-green-300" : "bg-gray-200"}`} />
                    )}
                  </div>
                  <div className="pb-4">
                    <p className={`text-[13px] font-medium leading-none mt-0.5 ${
                    ...
                    <p className="text-[11px] text-gray-400 mt-1">
                      {step.service}
                      {step.status === "in_progress" && <span className="text-[#3bcaca]"> · Running…</span>}
                      {step.status === "success" && <span className="text-green-500"> · Complete</span>}
            ...
            {state.type === "completed" && (
              <div className={`mt-2 pt-4 border-t border-gray-100 flex items-center gap-2 ${
                executingSteps.some((s) => s.status === "failed") ? "text-red-500" : "text-green-600"
              }`}>
                {executingSteps.some((s) => s.status === "failed") ? (
                  <>
                    ...
                    <span className="text-[13px] font-medium">Workflow completed with errors</span>
                  </>
                ) : (
                  <>
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span className="text-[13px] font-medium">Workflow completed successfully</span>
                  </>
                )}
              </div>
            )}
